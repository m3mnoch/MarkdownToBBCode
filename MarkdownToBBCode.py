#
# markdown replacement function originally found here:
# 	https://gist.github.com/sma/1513929
#
# note:
# 	only works on a subset of markdown.  if you're writing
#	links and images in markdown instead of bbcode, you're
#	probably doing it wrong.  formatting, and lists tho?
...
class MarkdowntobbcodeCommand(sublime_plugin.TextCommand):
	def run(self, edit):
		...
		regionString = markdown_to_bbcode(self.view.substr(allcontent))
	...
	def markdown_to_bbcode(s):
	    ...
	    def translate(p="%s", g=1):
	        def inline(m):
	            ...
	            #s = re.sub(r"\[(.*?)\]\[(.*?)\]", replace_link, s) # not sure why this hates links.  *shrug*
	    ...
	    s = re.sub(r"(?m)^(\S.*)\n=+\s*$", translate("~[size=20pt][b]%s[/b][/size]"), s)
	    s = re.sub(r"(?m)^(\S.*)\n-+\s*$", translate("~[size=14pt][b]%s[/b][/size]"), s)
	    s = re.sub(r"(?m)^#\s+(.*?)\s*#*$", translate("~[size=20pt][b]%s[/b][/size]"), s)
	    s = re.sub(r"(?m)^##\s+(.*?)\s*#*$", translate("~[size=14pt][b]%s[/b][/size]"), s)
